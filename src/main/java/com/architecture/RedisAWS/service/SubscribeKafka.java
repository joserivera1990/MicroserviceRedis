...
public class SubscribeKafka implements RequestHandler<Object, Object>{
...
public static final String KEY_SERIALIZAER = "org.apache.kafka.common.serialization.StringDeserializer";
	...
	public Object handleRequest(Object input, Context context) {
        ...
        try( KafkaConsumer<String, String> consumer = new KafkaConsumer<>( props )) { 
        	consumer.subscribe(Collections.singletonList("basic")); 
        	while (true) {
            	ConsumerRecords<String, String> records = consumer.poll(1000);
            	System.out.println(records.count() + "count");
            	for (ConsumerRecord<String, String> record : records) {
            		System.out.printf("offset = %d, key = %s, value = %s", record.offset(), record.key(), record.value());  
            		registerMessage.saveRegister(Integer.valueOf(1), record.value(), Integer.valueOf(record.key()), context);
            	}
        	}
        } 
    }	
